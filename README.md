...
**The purpose of this SDK is to make development of ExternalProcessors easy**. Specifically, this SDK _certainly_ won't supply the most _performant_ edge functions. Without a doubt that will come from eschewing the ease-of-use functionality here, implementing your filter in a compiled language, or even more likely _not even using an ExternalProcessor_ but instead using a custom filter binary or a WASM plugin. Performance isn't the goal; usability, maintainability, and low time-to-functionality is. 
